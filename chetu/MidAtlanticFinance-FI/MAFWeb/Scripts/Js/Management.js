...
$(document).ready(function () {
    ...
    $('#btnqueuesave').click(function () {
        ...
        // var Reportid = $('#Reportidhidden').val();
        var ScheduleQueue= "";
        $('.chkqueue:checked').map(function () { ScheduleQueue += this.value + ","; });
        $('#QueueNum' + UserReportid).val(ScheduleQueue);
    });
...
function AjaxScheduleUserList(Username, Abbreviation) {
    ...
    if (Abbreviation == "DNDStatus") { queuenum = $('#ddlQueue2').val(); }
    if (Abbreviation == '') { Abbreviation = currentAbbreviations }
    $.ajax({
        cache: false,
        type: "GET",
        ...
        success: function (data) {
            ...
            if (Abbreviation == "QueueLog" || Abbreviation=="DNDStatus") { if (Scheduleid == 0) { $('.chkuser').attr('checked', 'checked') } }
        },
        error: function (xhr, ajaxOptions, thrownError) {
            ShowErrorPoup('Failed to get schedule report list . Please refresh this page');
        }
...
// this function show the Schedule Queue list in Schedule Report by pop up
function ScheduleQueueList(Reportid, Abbreviation) {
    $('#ScheduleQueuelistpop').css('display', 'block');
    $('#Reportidhidden').val(Reportid);
    $('#txtschedulequeue').val('');
    UserReportid = Reportid;
    AjaxScheduleQueueList("", Abbreviation);
}

//Ajax function show the Schedule user list in Schedule Report in pop up
function AjaxScheduleQueueList(queueName, Abbreviation) {
    var Scheduleid = $('#SSID' + UserReportid).val().trim();
    $.ajax({
        cache: false,
        type: "GET",
        url: "/Management/PartialScheduleQueue",
        data: { "queueName": queueName, "scheduleId": Scheduleid },
        success: function (data) {
            $('#ScheduleQueuedata').html('');
            $('#ScheduleQueuedata').html(data);
        },
        error: function (xhr, ajaxOptions, thrownError) {
            ShowErrorPoup('Failed to get schedule report list . Please refresh this page');
        }
...
function UpdateMultiUserScheduleReports(reportid, Abbreviation) {
    var queuenum = "";
    var zone1FromTime;
    var zone1ToTime;
    var zone2FromTime;
    ...
    if (Abbreviation == 'QueueLog') { queuenum = $('#ddlQueue').val().trim(); }
    if (Abbreviation == 'DNDStatus') { queuenum = $('#ddlQueue2').val().trim(); }
    if (Abbreviation == 'HourlyCallLog' || Abbreviation == 'DropCallLog') { queuenum = $('#QueueNum'+reportid).val(); }
...
function DeleteSchedule(scheduleId) {
    $.ajax({
        cache: false,
        type: "GET",
        url: "/Management/Deleteschedule",
        data: { "scheduleId": scheduleId },
        success: function (data) {
        },
        error: function (xhr, ajaxOptions, thrownError) {
            ShowErrorPoup('Failed to delete schedule report . Please refresh this page');
        }
